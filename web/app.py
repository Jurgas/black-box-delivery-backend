...
def save_user(username, password, firstname, lastname, address):
    salt = gensalt(4)
...
@app.route('/sender/available', methods=["GET"])
...
def sender_create():
    ...
    if not save_user(username, password, firstname, lastname, address):
...
def sender_get_labels():
    ...
    return make_response("{" + f"{data}" + "}", 200)
